...
public class PathTileOverlay extends Overlay
{
	...
	private void renderCollisionMap(Graphics2D graphics)
	{
		...
		for (int sceneX = 0; sceneX < Perspective.SCENE_SIZE; sceneX++)
		{
			for (int sceneY = 0; sceneY < Perspective.SCENE_SIZE; sceneY++)
			{
				LocalPoint local = LocalPoint.fromScene(sceneX, sceneY, worldView.getScene());

				Polygon tilePolygon = Perspective.getCanvasTilePoly(client, local);
				...
				int location = WorldPointUtil.fromLocalInstance(client, local);
	...
	private void drawTransportInfo(Graphics2D graphics, PathStep currentStep, PathStep nextStep, List<PathStep> path, int pathIndex)
	{
		...
		// tile
		// PORT EDIT: the shim's getLocalPlayer() is null before the player has spawned (login, world
		// hop); upstream's is always non-null once a path exists. The path can outlive the player
		// across a hop, so guard rather than NPE the whole overlay for the frame.
